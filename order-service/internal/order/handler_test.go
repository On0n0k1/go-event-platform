...
type stubStore struct {
	createOrderFunc func(ctx context.Context, o Order, evt OutboxEvent) error
	getOrderFunc    func(ctx context.Context, id string) (Order, error)
	created         []Order
	outboxEvents    []OutboxEvent
}

func (s *stubStore) CreateOrder(ctx context.Context, o Order, evt OutboxEvent) error {
	// Mirrors real transactional atomicity: nothing is recorded unless the
	// whole operation succeeds.
	if err := s.createOrderFunc(ctx, o, evt); err != nil {
		return err
	}
	s.created = append(s.created, o)
	s.outboxEvents = append(s.outboxEvents, evt)
	return nil
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantStatus == http.StatusCreated {
				if len(store.outboxEvents) != 1 {
					t.Fatalf("outbox events written = %d, want 1", len(store.outboxEvents))
				}
				evt := store.outboxEvents[0]
				if evt.Subject != events.SubjectCreated {
					t.Errorf("outbox event subject = %q, want %q", evt.Subject, events.SubjectCreated)
				}
				...
				if decoded.SKU != "SKU-001" {
					t.Errorf("outbox payload sku = %q, want SKU-001", decoded.SKU)
				}
				if decoded.OrderID != store.created[0].ID {
					t.Errorf("outbox payload order_id = %q, want %q", decoded.OrderID, store.created[0].ID)
				}
			} else if len(store.outboxEvents) != 0 {
				t.Errorf("outbox events written = %d, want 0 for a non-created order", len(store.outboxEvents))
			}
		})
	}
}
