...
func TestClientLoadsValidCertKeyAndCA(t *testing.T) {
	...
	if cfg.RootCAs == nil {
		t.Error("RootCAs is nil, want a populated pool")
	}
}

func TestClientFailsOnMissingCertFile(t *testing.T) {
	_, keyPEM := generateSelfSignedCert(t)
	keyFile := writeTempFile(t, keyPEM)

	if _, err := Client(filepath.Join(t.TempDir(), "missing.crt"), keyFile, keyFile); err == nil {
		t.Fatal("expected error for missing cert file")
	}
}

func TestClientFailsOnMissingCAFile(t *testing.T) {
	certPEM, keyPEM := generateSelfSignedCert(t)
	certFile := writeTempFile(t, certPEM)
	keyFile := writeTempFile(t, keyPEM)

	if _, err := Client(certFile, keyFile, filepath.Join(t.TempDir(), "missing-ca.crt")); err == nil {
		t.Fatal("expected error for missing CA file")
	}
}

func TestClientFailsOnMalformedCA(t *testing.T) {
	certPEM, keyPEM := generateSelfSignedCert(t)
	certFile := writeTempFile(t, certPEM)
	keyFile := writeTempFile(t, keyPEM)
	badCAFile := writeTempFile(t, []byte("not a valid pem"))

	if _, err := Client(certFile, keyFile, badCAFile); err == nil {
